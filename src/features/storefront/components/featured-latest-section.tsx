...
export function FeaturedLatestSection({ products }: FeaturedLatestSectionProps) {
      ...
      <SectionContainer>
        <div className="flex flex-col md:flex-row md:items-end justify-between mb-12 gap-6 ">
         ...
         <div className="flex gap-4">
          {[{ id: "1", label: "NEW ARRIVALS" , value: "new-arrivals" }, { id: "2", label: "FEATURED PRODUCTS", value: "featured-products" }].map((tab) => (
          <Button
          key={tab.id}
          onClick={() => setIsActive(tab.value)}
          variant="outline" asChild className={` p-6 text-md tracking-wide ${isActive===tab.value ? "text-background hover:bg-primary hover:text-background bg-primary" : ""}`}>
            <Link href="/products">
              {tab?.label}
